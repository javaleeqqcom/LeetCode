...
class SafeIterBase2(Generic[T_Node]):
    ...
    @classmethod
    def _flatten(cls, it:SafeIterBase2, max_len: int = -1) -> List[KitBase2[T_Node]]|List[T_Node]:
        """
        安全展开链表，返回包装节点列表。
        ...
        for i,node in enumerate(it): 
            if i == max_len: # i 是逐一递增的，若 max_len 非负，则必能生效
                break
            else:
                nodes.append(node)
        return nodes
    ...
    def _to_raw_list(cls, kit_nodes: List[KitBase2[T_Node]]) -> List[T_Node]:
        ...
        assert len(res) == len(kit_nodes), "展开过程中发现空节点，设计错误或数据遭到破坏！"
        return res
