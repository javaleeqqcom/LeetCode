...
class SafeIterBase(Iterator[Tuple[int, T]]):
    def __init__(self, init_node: Optional[T] = None, init_idx: int = 0, early_stop: bool = not __DEBUG__):
        ...
        # 记录所有导致冲突的【原始节点索引】
        self._repeat_indices = defaultdict(List[int]) # 首次出现时的 assigned_idx -> [重复访问时的 assigned_idx 列表]
...
# HasLR = TypeVar("T_LR",bound=HasLR)

class LayeredTraversal(SafeIterBase):
    def __init__(self, root: Optional[HasLR], early_stop: bool = not __DEBUG__):
        super().__init__(init_node=root, init_idx=1, early_stop=early_stop)
        self._queue = deque()

    def _push_safe(self,idx:int,node:Optional[HasLR]):
...
class HeapRoute(SafeIterBase):
    ...
    def __init__(self, init_node: HasLR, heap_index: int):
...
class TreeNodeKitBase(KitBase[HasLR]):
    ...
    def flatten(self,max_depth:int|None = None) -> Tuple[List[Tuple[int, HasLR]], Optional[int]]:
    ...
    def layer_iter(self) -> SafeIterBase[HasLR]:
        """调用 SafeIter 安全地层序遍历，遍历完毕或出现重复节点时停止"""
        return LayeredTraversal[HasLR](self._node)
    
    def NLR_iter(self) -> SafeIterBase[HasLR]:
        """前序遍历迭代器 (NLR)"""
        return PreorderTraversal[HasLR](self._node)

    def LNR_iter(self) -> SafeIterBase[HasLR]:
        """中序遍历迭代器 (LNR)"""
        return InorderTraversal[HasLR](self._node)

    def LRN_iter(self) -> SafeIterBase[HasLR]:
        """后序遍历迭代器 (LRN)"""
        return PostorderTraversal[HasLR](self._node)
    ...
    @classmethod
    def _to_string(cls, root: TreeNodeKitBase | HasLR | None,
        ...
        """
        node: Optional[HasLR] = cls.unwrap(root)
        if node is None:
            return "<class 'TreeNodeKit'>: empty"

        # 根据 full_traversal 决定 early_stop 行为
        it = LayeredTraversal[HasLR](node, early_stop = not full_traversal)
        ...
        # 获取重复节点的第一次索引列表（去重）
        repeat_idxs = np.unique(it.repeat_idx).tolist()
        if __DEBUG__:
            assert repeat_idxs == sorted(repeat_idxs)

        # 构建索引到节点值的映射
        idx_val = {idx: getattr(n, prep_property) for idx, n in idx_node}

        repeat_idx_dict = {}
        for idx in it.repeat_idx:
            repeat_node_id = id(TreeNodeKitBase(root).get_heap(idx).node)
            head_node_idxs = it._seen[repeat_node_id]
            assert head_node_idxs[0] == idx

            if idx-1 not in repeat_idx_dict:
                repeat_idx_dict[idx] = f"*{idx}" # 表示重复点节点

            for head_idx in head_node_idxs[1:]:
                if head_idx not in repeat_idx_dict:
                    repeat_idx_dict[head_idx] = f"^{idx}" # 表示非法链指向该重复点节点
        ...
        if full_traversal:
            ...
            stop_idx = repeat_idxs[0] if repeat_idxs else None
...
class DfsTreeTraversal(SafeIterBase[HasLR]):
    def __init__(self, root: Optional[HasLR]):
        super().__init__(root, 1)
        # 栈存储 (索引, 节点, 是否已访问子节点)
        self._stack: List[Tuple[int, HasLR, Any]] = []

    def _push_safe(self, idx: int, node: Optional[HasLR], *args):
...
class InorderTraversal(SafeIterBase[HasLR]):
    """中序遍历迭代器 (LNR)"""
    def __init__(self, root: Optional[HasLR]):
    ...
    def _push_left(self, idx: int, node: HasLR):
        """下探左子树，入栈即登记"""
        curr = node
        curr_idx = idx
        while curr.left:
...
class PostorderTraversal(SafeIterBase[HasLR]):
    """后序遍历迭代器 (LRN)"""
    def __init__(self, root: Optional[HasLR]):
        super().__init__(None, 1)
        # 栈存储 (索引, 节点, 是否已访问子节点)
        self._stack: List[Tuple[int, HasLR, bool]] = []
        
        if root and self._check_safe(1, root):
            self._stack.append((1, root, False))
            self._current_node = self._find_next_post_node()
        
    def _find_next_post_node(self) -> Optional[HasLR]:
        while self._stack:
