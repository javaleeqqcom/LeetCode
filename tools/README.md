...
2. ** KitBase **
   ...
   - 
3. ** 进一步性能优化 **
   - 树节点的 visit_idx 因为（堆索引方式），只需要做如下运算： ==、!=、+1、<<1（或*2）、>>1（或//2）、%2（判断奇偶）。拟采用带头结点的链表：头结点（链表首指针，链表长，首指针位数）、链表节点（next指针，val无符号整型）。其中 链表首 为最低位，以便于 <<1 时，当首指针位数<整型位数，只需要修改首指针.val << 1即可。而无需修改其他指针，+1同理，只有当 val 溢出，才用头插法创建新指针。
4. **统一安全迭代器接口**
