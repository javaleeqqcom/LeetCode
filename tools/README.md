...
4. **多进程加速、早停机制**
   ...
   采用 Python3.14 的独立GIL 多线程： concurrent.futures + concurrent.interpreters 实现
   主线程作为生产者分割测试用例，子线程作为消费者执行测试
   - 采用等比递减分割器分割测试用例
   - 子线程采用 error_queue 传递报错信息，以便所有线程收到错误信息后可以停止执行
   - 子线程采用 wrong_queue 汇总错误结果（当具有 expected 时），以便错误结果超出阈值可以早停
5. **VS Code 插件集成**  
