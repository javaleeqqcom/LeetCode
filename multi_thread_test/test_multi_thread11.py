...
def execute_in_interpreter(test_cases: List[int]) -> List[bool]:
    ...
    print(f"Obj:{id(_solution)} called {len(test_cases)} test cost: {end-begin:.6f} s")
    return res
...
def main():
    ...
    with concurrent.futures.InterpreterPoolExecutor(max_workers=12) as executor:
        results_parallel = list(chain.from_iterable(
            executor.map(
                execute_in_interpreter,
                [test_cases[split_idx[thread_id]:split_idx[thread_id+1]] for thread_id in range(_N_THREAD_)]
            )
        ))
