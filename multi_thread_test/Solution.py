...
def generate_primes(limit: int = 65536) -> Tuple:
    """使用埃拉托斯特尼筛法生成素数表"""
    sieve = [True] * (limit + 1)
...
class Solution:
    def __init__(self) -> None:
        self.prime = _prime # tuple(_prime)
    ...
    def is_sqrt_prime(self, num: int) -> bool:
        ...
        def is_prime(num: int ,hi:Optional[int] = None) -> int:
            ...
            if num <= self.prime[-1]:
                # 使用 bisect 进行二分查找
                pos = bisect.bisect_left( self.prime , num , hi=hi)
                if pos < len(self.prime) and self.prime[pos] == num:
                    return pos
                else:
                    return -1 
