    ...
    while early_stop_queue.empty():
        ...
        for num in cases:
            try:
                ...
                is_early_stop = True
                break # early_stop_queue.empty() 不成立自然会退出外循环

        results.append(( group_id,  results_buff ))

    if not is_early_stop:
        early_stop_queue.put(None) # 正常结束则以 None 标记
...
def main():
    ...
    # 使用归并排序合并结果
    results_parallel = chain.from_iterable(merge_sorted_lists(
        results_parallel,
