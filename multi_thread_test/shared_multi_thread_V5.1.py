...
def execute_in_interpreter(interpreter_id : int , test_queue :interpreters.Queue)->List[Tuple[int,Any]]:
    """在子解释器中执行测试用例"""
...
def main():
    ...
    with concurrent.futures.InterpreterPoolExecutor(max_workers=_N_CORE_) as executor:
        ...
        func = partial(execute_in_interpreter, test_queue=test_queue)
