...
class Solution:
    def minimumPairRemoval(self, nums: List[int]) -> int:
        ...
        # 用原位双向链表存储操作后的 nums，双哨兵防止越界，用 inf,max_num 确保越界判断
        link = [Node(-inf,1,0)]+ [Node(a,i+2 if i<n-1 else 0,i) for i,a in enumerate(nums)] + [Node(inf,n+1,n)]
        ...
        while dec_times>0 and H: # 当还有递减次数 且 堆不为空时循环    
            ...
            if aj_sum != L.val + R.val: # 元素值已经被修改过，则跳过（被删除的元素会标记为无穷大，数据确保不会累积出现相等的结果）
                ...
                new_items = list(filter(lambda x: abs(x[0]) < max_num, [( LL_val + L_val, L.pre) ,(L_val + RR_val, i)])) # 过滤掉边界节点
