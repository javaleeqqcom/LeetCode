...
def parse_input_file(filepath: str) -> tuple[bool, list | dict]:
    """解析输入文件"""
    ...
    if has_eq:
        ...
        for line in lines:
            if '=' not in line:
                raise ValueError(f"Line does not contain '=' but other lines do: {line}")
            ...
            try:
                ...
                raise ValueError(f"Failed to parse value in line '{line}': {e}")
        ...
        for line in lines:
            try:
                parsed_val = eval(line.strip(), {"__builtins__": {}}, {})
            except Exception as e:
                raise ValueError(f"Failed to parse line as Python literal: '{line}' ({e})")
