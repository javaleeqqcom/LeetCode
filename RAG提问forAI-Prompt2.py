...
template_text = fr"""
你的分析基本正确，不过改进需要逐步实现。
solution_runner 是绑定执行代码的，目前仅设计了 Python，因为要先解析 Solution 类的结构，才能自动检查代入的参数是否出错。
...
现在请设计 Solution_struct.py 的代码，并说明 solution_runner.py 需要修改的函数。
为下一步实现 case_generator 做准备。
"""
